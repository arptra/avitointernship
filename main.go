...
func getInfo() info {
	...
	vectorBan := []infoBan{}
	i := 1
	for subnet, v := range ratelimit.Visitors {
		if v.Restrict == true {
			ban := infoBan{}
			ban.Subnet = subnet
			ban.Id = i
	...
	ratelimit.Mu.Unlock()
	IpsBan := info{conections, vectorBan, ratelimit.InitSettings()}
	return IpsBan
}
...
func DeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		ratelimit.Mu.Lock()
		delete(ratelimit.Visitors, address)
		ratelimit.Mu.Unlock()
...
func main() {
	params := ratelimit.InitSettings()
	fmt.Println("Server is listening...")
	router := mux.NewRouter()
	router.HandleFunc("/", infoHandler)
	router.HandleFunc("/delete", DeleteHandler)
	http.Handle("/", router)
	http.ListenAndServe("localhost:8181", ratelimit.Limit(router, params))
}
